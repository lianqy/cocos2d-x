...
bool HelloWorld::init()
{
	...
	float tmxw = tmx->getContentSize().width;
	float tmxh = tmx->getContentSize().height;
	tmx->setScaleX(winw / tmxw);
	tmx->setScaleY(winh / tmxh);
	...
	time = Label::createWithTTF(ttfConfig, std::to_string(killNum));
	time->setPosition(Vec2(origin.x + visibleSize.width / 2,
		origin.y + visibleSize.height - time->getContentSize().height));
	schedule(schedule_selector(HelloWorld::creatMonster), 3.0f, kRepeatForever, 0);
...
void HelloWorld::moveEvent_w(Ref* pSender) {
	if (player->getSpriteFrame() == frame0 && pT->getPercentage()) {
		...
		if (winh - nowy - playh / 2 >= 50) {
			player->runAction(Spawn::create(animate, MoveBy::create(0.25f, Vec2(0, 50)), nullptr));
		} else {
			player->runAction(Spawn::create(animate, MoveBy::create(0.25f, Vec2(0, winh - nowy - playh / 2)), nullptr));
		}
	}
}
...
void HelloWorld::attackEvent(Ref* pSender) {
	if (player->getSpriteFrame() == frame0 && pT->getPercentage()) {
		...
		if (attackMonster()) {
			pT->setPercentage(pT->getPercentage() + 15.0);
			killNum++;
			time->setString(std::to_string(killNum));
			database->setIntegerForKey("killNum", killNum);
		}
	}
}
...
void HelloWorld::moveEvent_a(Ref* pSender) {
	if (player->getSpriteFrame() == frame0 && pT->getPercentage()) {
		...
		if (nowx - playw / 2 - 50 >= 0) {
			player->runAction(Spawn::create(animate, MoveBy::create(0.25f, Vec2(-50, 0)), nullptr));
		}
		else {
			player->runAction(Spawn::create(animate, MoveBy::create(0.25f, Vec2(-(nowx - playw / 2), 0)), nullptr));
		}
	}
}
...
void HelloWorld::moveEvent_s(Ref* pSender) {
	if (player->getSpriteFrame() == frame0 && pT->getPercentage()) {
		...
		if (nowy - playh / 2 - 50 >= 0) {
			...
			player->runAction(Spawn::create(animate, MoveBy::create(0.25f, Vec2(0, -(nowy - playh / 2))), nullptr));
		}
	}
}
...
void HelloWorld::moveEvent_d(Ref* pSender) {
	if (player->getSpriteFrame() == frame0 && pT->getPercentage()) {
		...
		if (winw - nowx - playw / 2 >= 50) {
			...
			player->runAction(Spawn::create(animate, MoveBy::create(0.25f, Vec2(winw - nowx - playw / 2, 0)), nullptr));
		}
	}
}
